...
indicators_c::indicators_c(){
    ...
    m_handler.indcs_var.ticks_out_vars = std::make_unique<ticks_out_t>(m_handler, m_indc_config.m_BWFS_config); 
    m_handler.indcs_var.qtys_in_vars = std::make_unique<qty_in_t>(m_handler, m_indc_config.m_BWFS_config); 
    m_handler.indcs_var.qty_out_vars = std::make_unique<qty_out_t>(m_handler, m_indc_config.m_BWFS_config); 
    m_handler.indcs_var.avrg_in_vars = std::make_unique<avrg_in_t>(m_handler, m_indc_config.m_BWFS_config); 
    m_handler.indcs_var.avrg_out_vars = std::make_unique<avrg_out_t>(m_handler, m_indc_config.m_BWFS_config); 
    m_handler.indcs_var.qty_in_out_vars = std::make_unique<qty_in_out_t>(m_handler, m_indc_config.m_BWFS_config); 
    m_handler.indcs_var.ticks_in_out_vars = std::make_unique<ticks_in_out_t>(m_handler, m_indc_config.m_BWFS_config); 
    m_handler.indcs_var.buy_vs_sell_vars = std::make_unique<buy_vs_sell_t>(m_handler, m_indc_config.m_BWFS_config); 
    m_handler.indcs_var.ema_vars = std::make_unique<ema_t>(m_handler, m_indc_config.m_ema_config); 
    m_handler.indcs_var.sma_vars = std::make_unique<sma_t>(m_handler, m_indc_config.m_sma_config); 
...
void indicators_c::set(const std::vector<std::vector<std::string>> &indcs){
    ...
    m_indc_config.set(indcs, indcs_state, num_of_indcs_per_mngr);

    m_indcs_trade_mngr = new ind_mngr_c<trade_stream_d,
                                                    indicator_t>(
                                        num_of_indcs_per_mngr[(uint64_t)source_e::SRC_TRADE]
                                        );

    m_indcs_kline_mngr = new ind_mngr_c<kline_d, indicator_t>(
