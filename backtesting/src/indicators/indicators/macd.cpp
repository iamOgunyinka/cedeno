...
static void calculate_macd(indicators::macd_t &handler){
    ...
    inf_macd.macd = handler.ema_q[handler.configuration->high_period] - 
                    handler.ema_q[handler.configuration->low_period];

    inf_macd.signal = handler.ema_q[handler.configuration->signal_period];

    inf_macd.histogram = inf_macd.macd - inf_macd.signal;
}
...
indicators::conf_macd_t get_config(const std::vector<std::string> &indcs){
    ...
    if(indcs.size() > 1){
        if(indcs.size() != 4)
            std::__throw_runtime_error("MACD config parameters incomplete");
        std::for_each(indcs.begin() + 1, indcs.end(), [&](const std::string &str){
            ...
            if(config_pair.first == "signal"){
                config.signal_period = strtoul(config_pair.second.c_str(), nullptr, 10);
            }else if(config_pair.first == "high"){
