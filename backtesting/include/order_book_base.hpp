...
/// an internal structure used for sort order book entry from lower to higher
/// price
...
/// an internal structure used for sort order book entry from higher to lower
/// price
...
class order_book_base_t {
public:
  /// place an order on the order book (for a market order, it isn't added to
  /// the book) \param orderBook - an order book that is meant to handle spot or
  /// futures trading \param order - an order to BUY/SELL
  friend void
  matching_engine::placeOrder(backtesting::order_book_base_t &orderBook,
                              backtesting::order_data_t const &order);

  /// cancel an order already on the order book
  /// \param orderBook - an order book that is meant to handle spot or futures
  /// trading \param order - an order to cancel
  friend void
  ...
  /// an asynchronous function called to kickstart the order book and get it
  /// running
  void run();
...
protected:
  ...
  /// for spot order book, this is the amount the user is spending on buying or
  /// selling \param order the order data signifying what to buy or sell \return
  /// the list of trades that occurred
  [[nodiscard]] virtual trade_list_t
  ...
  /// \return a (possible) list of all the trades that was bought on the other
  /// side of the trade
