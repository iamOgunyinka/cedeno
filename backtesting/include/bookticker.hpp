...
/// This is an internal structure. It is read from the file and processed internally
struct binance_bktick_data_t {
  ...
  /// internal function responsible for reading data from file using the dataStreamer
  /// \param dataStreamer a stream object responsible for reading data from file
  /// \return a `binance_bktick_data_t` object
...
/// This is the configuration sent from the user (most likely from the Python script)
...
struct bktick_config_t {
  ...
  bktick_callback_t callback = nullptr; //!< the callback needed for periodic updates
...
/// checkAndValidateBookTickerRequestHelper - an internal helper function to check that
/// a user configuration is valid
...
/// checkAndValidateBookTickerRequest - an internal function to check that a user
...
/// bookTickerProcessingThreadImpl - an internal function that reads files from disk,
/// process it and does some in-house processing
/// \param config - the user configuration
...
/// bookTickerChildThreadImpl - an internal function, it is used by a thread to read
/// book ticker from file and preprocess it for further processing
